...
import { layoutGroups, questions } from '../data/questionBank.js'
import {
  normalizeHomeworkItems,
  publishHomeworkToStore,
  saveDraftToStore
} from '../data/homeworkStore.js'

const TYPE_ORDER = ['选择题', '填空题', '判断题', '计算题', '解决问题']
...
const TEMPLATE_PRESET = {
  作业: { 主标题: true, 副标题: false, 学校姓名栏: true, 答题栏: false, 装订线: false, 注意事项: false },
  测试: { 主标题: true, 副标题: true, 学校姓名栏: true, 答题栏: true, 装订线: false, 注意事项: true },
  考试: { 主标题: true, 副标题: true, 学校姓名栏: true, 答题栏: true, 装订线: true, 注意事项: true }
}
const cn = (i) => CN_NUM[i] || (i + 1)
const toType = (t) => (t === '解答题' ? '解决问题' : t)

const fallbackItems = layoutGroups.flatMap(g => g.items.map(q => ({ ...q, group: g.type })))
...
export default function HomeworkLayout() {
  ...
  const [items, setItems] = useState(() => getInitialItems(state))
  const [activeId, setActiveId] = useState(() => getInitialItems(state)[0]?.id ?? null)
  ...
  const applyTemplate = (t) => { setTemplate(t); setShow(TEMPLATE_PRESET[t]) }
  const toggleItemAnswer = (id) => setItems(it => it.map(q => q.id === id ? { ...q, answerArea: !q.answerArea } : q))
  const swapItem = (id) => {
    const i = items.findIndex(q => q.id === id)
    if (i < 0) return
    const cur = items[i]
    const used = new Set(items.map(q => q.id))
    const cand = questions.find(q => !used.has(q.id) && toType(q.type) === cur.type) || questions.find(q => !used.has(q.id))
    if (!cand) { flashToast('题库暂无可替换的题') ; return }
    const replacement = normalizeHomeworkItems([{ ...cand, group: cur.group, score: cur.score }])[0]
    setItems(it => it.map(q => q.id === id ? replacement : q))
    setActiveId(replacement.id)
    flashToast('已换为同类型的另一道题')
  }
  ...
  useEffect(() => {
    ...
    if (!items.some(q => q.id === activeId)) setActiveId(items[0].id)
  }, [items, activeId])

  const setScore = (id, score) => setItems(it => it.map(q => q.id === id ? { ...q, score: Math.max(0, Number(score) || 0) } : q))
  const removeItem = (id) => setItems(it => it.filter(q => q.id !== id))
  const moveItem = (id, dir) => {
    setItems(it => {
      const idx = it.findIndex(q => q.id === id)
      const swap = idx + dir
      if (idx < 0 || swap < 0 || swap >= it.length || groupKey(it[swap], groupMode) !== groupKey(it[idx], groupMode)) return it
  ...
  const onDrop = (targetId) => {
    ...
    // 按题型分组时仅允许同组内拖拽，避免拖拽错误改变题型归类
    if (groupMode === '按题型' && groupKey(items[from], groupMode) !== groupKey(items[to], groupMode)) return
    const next = [...items]
  ...
  const finish = (msg) => { setToast(msg); setTimeout(() => navigate('/homework', { state: { toast: msg } }), 900) }
  const saveDraft = () => {
    if (items.length === 0) {
      setToast('请先添加题目')
      return
    }
    saveDraftToStore({ id: draftId, name: name.trim() || '未命名作业', items })
    finish('已保存草稿，可在「未发布」中继续编辑')
  }
  const publish = () => {
    if (items.length === 0) {
      setToast('请先添加题目')
    ...
    publishHomeworkToStore({ id: draftId, name: name.trim() || '未命名作业', items, classes: [...classes], deadline })
    finish(`已发布给 ${[...classes].join('、') || '所选班级'}`)
  }
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col bg-slate-50">
      ...
      <div className="px-6 py-3 bg-white border-b border-slate-100 flex items-center gap-4 shrink-0">
        <button onClick={() => navigate('/homework/select', { state: { id: draftId, name, items, selectedIds: items.map(q => q.id) } })} className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-500">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative">
            <button onClick={() => setPublishOpen(o => !o)} disabled={items.length === 0} className="btn-primary h-9 disabled:opacity-40">发布作业 <ChevronDown className="w-4 h-4" /></button>
            {publishOpen && (
        ...
        <div className="overflow-y-auto">
          <div className="mx-auto max-w-[760px] bg-white rounded-lg shadow-card border border-slate-100 px-12 py-10 relative">
            ...
            <div className="mt-5 space-y-5">
              {groups.length === 0 && (
                <div className="py-20 text-center text-sm text-slate-400">暂无题目，请返回选题页添加题目</div>
              )}
              {groups.map((g, gi) => (
                <div key={g.type}>
                  <div className="text-[15px] font-semibold text-slate-800 mb-2">{cn(gi)}、{g.type}</div>
                        ...
                        >
                          {active && (
                            <div className="absolute -top-3 left-3 right-3 z-10 flex items-center gap-0.5 bg-white border border-brand-200 rounded-md shadow-sm px-1 py-1 text-xs text-slate-600 flex-wrap">
                              <ToolBtn icon={<RefreshCw className="w-3 h-3" />} label="换题" onClick={() => swapItem(q.id)} />
                              <ToolBtn icon={<AlertCircle className="w-3 h-3" />} label="纠错" onClick={() => flashToast('已记录纠错反馈，教研组将复核')} />
                              <ToolBtn icon={<Eye className="w-3 h-3" />} label="详情" onClick={() => flashToast(`知识点：${q.kp} · 难度：${q.diff} · 来源：${q.source || '校本题库'}`)} />
                              <ToolBtn icon={<SquarePen className="w-3 h-3" />} label={q.answerArea ? '去掉作答区' : '插入作答区'} onClick={() => toggleItemAnswer(q.id)} />
        ...
        <div className="overflow-y-auto space-y-4">
            ...
            <div className="flex gap-4 mb-4">
              {TEMPLATES.map(t => (
                <label key={t} className="flex items-center gap-1.5 text-sm text-slate-700 cursor-pointer">
                  <input type="radio" name="tpl" checked={template === t} onChange={() => applyTemplate(t)} className="accent-brand-600" />{t}
                </label>
              ))}
            </div>
          ...
          <div className="card p-4">
            ...
            <div className="space-y-3">
              {groups.map((g, gi) => (
                <div key={g.type}>
                  <div className="text-xs text-slate-500 mb-1.5">{cn(gi)}、{g.type}</div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 space-y-2">
              ...
              <button onClick={() => window.print()} className="text-brand-600 text-xs flex items-center gap-1 hover:underline"><Printer className="w-3.5 h-3.5" /> 预览打印效果 ›</button>
            </div>
          </div>
        </div>
      </div>
...
function getInitialItems(state) {
  if (Array.isArray(state?.items)) return normalizeHomeworkItems(state.items)
  if (Array.isArray(state?.selectedIds)) {
    const selected = questions.filter(q => state.selectedIds.includes(q.id))
    return normalizeHomeworkItems(selected)
  }
  return normalizeHomeworkItems(fallbackItems)
}

function groupKey(item, mode) {
  if (mode === '按知识点') return item.kp || '未归类知识点'
  if (mode === '按加入顺序') return '作业题目'
  return item.group || item.type || '未分组'
}
...
function buildGroups(items, mode) {
  ...
  if (mode !== '按题型') return [...map.entries()].map(([type, groupItems]) => ({ type, items: groupItems }))

  const orderedKeys = [
    ...TYPE_ORDER.filter(type => map.has(type)),
    ...[...map.keys()].filter(type => !TYPE_ORDER.includes(type))
  ]
  return orderedKeys.map(type => ({ type, items: map.get(type) }))
}
