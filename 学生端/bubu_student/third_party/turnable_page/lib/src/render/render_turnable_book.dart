...
class RenderTurnableBook extends RenderBox
  ...
  int? _primaryPointer;
  bool _blockedByMultiTouch = false;
  ...
  void render(double timer) {
    ...
    if (currentAnimation != null) {
      double elapsed = _timeMs - currentAnimation.startedAt;
      if (elapsed < 0) elapsed = 0;
      final frameIndex = (elapsed / currentAnimation.durationFrame).floor();
      final lastFrameIndex = currentAnimation.frames.length - 1;
      // The terminal fold geometry can collapse to a thin clip and expose the
      // old static spread for one frame, so commit the page turn before it paints.
      if (lastFrameIndex < 0 || frameIndex >= lastFrameIndex) {
        _completeAnimation(currentAnimation);
      } else {
  ...
  void _completeAnimation(AnimationProcess completedAnimation) {
    if (animation != completedAnimation) return;
    completedAnimation.onAnimateEnd();
  ...
  void finishAnimation() {
    final currentAnimation = animation;
    if (currentAnimation != null) {
      if (currentAnimation.frames.isNotEmpty) {
        currentAnimation.frames.last();
      }
      _completeAnimation(currentAnimation);
    }
  }
  ...
  void handleEvent(PointerEvent event, HitTestEntry entry) {
    if (event.kind == ui.PointerDeviceKind.stylus ||
        ...
        !settings.enableUserInteraction) {
    ...
    if (event is PointerDownEvent) {
      ...
      if (_blockedByMultiTouch || event.pointer != _primaryPointer) return;
      _handlePointerMove(getPointerOffset(event: event));
    } else if (event is PointerUpEvent || event is PointerCancelEvent) {
      if (event.pointer == _primaryPointer && !_blockedByMultiTouch) {
        _handlePointerUp(getPointerOffset(event: event));
      }
      _activePointers.remove(event.pointer);
      if (_activePointers.isEmpty) {
        _blockedByMultiTouch = false;
  ...
  void _handlePointerMove(Offset position) {
    ...
    if (_initialTouchPoint != null && !_dragRejected) {
      final deltaX = (point.x - _initialTouchPoint!.x).abs();
      final deltaY = (point.y - _initialTouchPoint!.y).abs();

      if (!_isDragging && _isVerticalScrollIntent(deltaX, deltaY)) {
        _dragRejected = true;
        return;
      }

      if (!_isDragging && _isHorizontalPageTurnIntent(deltaX, deltaY)) {
        _isDragging = true;
        if (!pageFlip.isUserTouch) {
          pageFlip.startUserTouch(_initialTouchPoint!);
          ensureAnimating();
        }
      }

      if (_isDragging && pageFlip.isUserTouch) ensureAnimating();
    }

    // Process move only after this render object owns a page flip gesture.
    if (pageFlip.isUserTouch) {
  ...
  void _handlePointerUp(Offset position) {
    ...
    if (!pageFlip.isUserTouch) {
      if (!_blockedByMultiTouch &&
          !_dragRejected &&
          _touchPoint != null &&
          _isEdgeTap(point)) {
        _processTapFlip(point);
        _resetGestureState();
        return;
      }
      _resetGestureState();
  ...
  bool _isHorizontalPageTurnIntent(double deltaX, double deltaY) {
    return deltaX > _minMoveThreshold && deltaX > deltaY * 1.18;
  }

  bool _isVerticalScrollIntent(double deltaX, double deltaY) {
    return deltaY > _minMoveThreshold && deltaY > deltaX * 1.18;
  }

  bool _isEdgeTap(model.Point point) {
    final rect = getRect();
    ...
    final localX = point.x - rect.left;
    final edgeWidth = math.min(96.0, math.max(48.0, rect.pageWidth * 0.16));
    return localX <= edgeWidth || localX >= rect.width - edgeWidth;
  }

  void _processTapFlip(model.Point point) {
    final rect = getRect();
    final corner = point.y - rect.top < rect.height * 0.5
        ? FlipCorner.top
        : FlipCorner.bottom;
    final localX = point.x - rect.left;
  ...
  void _cancelUserGesture() {
    ...
    pageFlip.flipProcess.reset();
    pageFlip.flipProcess.setState(FlippingState.read);
    setBottomPage(null);
